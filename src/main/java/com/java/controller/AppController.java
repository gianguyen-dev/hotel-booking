...
import com.java.service.MomoService;
...
public class AppController {
	...
	public String showLoginPage(HttpSession session) {
		if (session.getAttribute("nameOfUser") == null || session.getAttribute("admin") == null) {
			return "login";
		} else {
	...
	public ModelAndView submitLogin(Model model, @ModelAttribute(value = "user") User user, HttpSession session) {
	...
	public String editCustomer(Model model, HttpSession session, @RequestParam("fullName") String fullName,
		...
		if (fullName.equals("") && birthday.equals("") && email.equals("") && address.equals("") && phone.equals("")
				&& identity.equals("")) {
...
//		model.addAttribute("listRooms", service.getAll());
		if (session.getAttribute("nameOfUser") != null) {
			...
			if (room_type.equals("phong2nguoi")) {
				...
				model.addAttribute("listRoomsBySix", roomService.getAllBySix(roomService.filterByRoomType("Phòng VIP")));
				...
				model.addAttribute("listRoomsBySix", roomService.getAllBySix(roomService.filterByRoomPrice(100000, 300000)));
			} else if (room_type.equals("between301kand400k")) {
				model.addAttribute("listRooms", roomService.filterByRoomPrice(301000, 400000));
				model.addAttribute("listRoomsBySix", roomService.getAllBySix(roomService.filterByRoomPrice(301000, 400000)));
			}
			return "home";
		}else {
			if (room_type.equals("phong2nguoi")) {
	...
	public ModelAndView showBookingForm(@PathVariable(name = "room_id") int room_id, HttpSession session) {
	...
	public String saveBooking(@ModelAttribute Booking booking, Model model, HttpSession session,
		...
		model.addAttribute("user_id", (long) user_id);
		...
		if (booking.getCheckin_date() == null) {
			errorMessage.add("Ngày vào không hợp lệ");
		}
		if (booking.getCheckout_date() == null) {
			errorMessage.add("Ngày ra không hợp lệ");
		}
...
//		paymentService.save(payment);	
	...
	public String confirmMomo(@RequestParam("phone") String phone, @RequestParam("ccv") String ccv, 
			@RequestParam("amount") String price_str, @RequestParam("payment_id") String payment_id_str, Model model, RedirectAttributes redirectAttributes, HttpSession session) {
		if (session.getAttribute("nameOfUser") == null) {
			return "redirect:/login";
		}
	...
	public String confirmBanking(@RequestParam("id_card") String id_card_str, @RequestParam("ccv") String ccv,
			RedirectAttributes redirectAttributes, @RequestParam("amount") String price_str,
			@RequestParam("payment_id") String payment_id_str, Model model) {
